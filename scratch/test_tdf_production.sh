#!/usr/bin/env bash
# One-shot, owner-authorized TDF PRODUCTION /Create contract probe.
# Usage: bash scratch/test_tdf_production.sh 1.00
# Credentials, test card, and CVV are requested interactively; never paste them into a command line.
set -euo pipefail
set +x
umask 077

base_url='https://api.thedonorsfund.org/thedonorsfund/integration'
...
api_key="${TDF_PRODUCTION_API_KEY:-}"
validation_token="${TDF_PRODUCTION_VALIDATION_TOKEN:-}"
card_number="${TDF_TEST_CARD:-}"
card_cvv="${TDF_TEST_CVV:-}"

if [[ ! -t 0 && "${TDF_ALLOW_NONINTERACTIVE:-0}" != "1" ]]; then
  printf 'Run interactively to enter production credentials, test card, and CVV. No request sent.\n' >&2
  exit 1
fi
if [[ -z "$api_key" ]]; then
  read -r -p 'TDF production Api-Key: ' api_key
fi
if [[ -z "$validation_token" ]]; then
  read -r -s -p 'TDF production Validation-Token: ' validation_token
  printf '\n'
fi
if [[ ! "$api_key" =~ ^[A-Za-z0-9_-]+$ || ! "$validation_token" =~ ^[A-Za-z0-9_-]+$ ]]; then
  printf 'Production Api-Key and Validation-Token must be set to valid values. No request sent.\n' >&2
  exit 1
fi

if [[ -z "$card_number" ]]; then
  read -r -s -p 'TDF production test card number: ' card_number
  printf '\n'
fi
if [[ -z "$card_cvv" ]]; then
  read -r -s -p 'TDF production test card CVV: ' card_cvv
  printf '\n'
fi
...
tmp_dir="$(mktemp -d)"
trap 'unset card_number card_cvv TDF_TEST_CARD TDF_TEST_CVV TDF_PRODUCTION_VALIDATION_TOKEN; rm -f "$tmp_dir/response.json" "$tmp_dir/curl.conf"; rmdir "$tmp_dir"' EXIT
...
payload = {
    ...
    'purposeType': 'Other',
    'purposeNote': 'Notzer Chesed controlled production API test',
}
...
printf 'Submitting one %s USD grant to account %s on TDF PRODUCTION. No automatic retry.\n' "$amount" "$account_number"
...
confirmation = (nested.get('confirmationNumber') or nested.get('ConfirmationNumber')
                or body.get('confirmationNumber') or body.get('ConfirmationNumber')
                or (body.get('data') if isinstance(body.get('data'), str) and body['data'].isdigit() else None))
transaction = nested.get('transactionId') or nested.get('TransactionId') or body.get('transactionId') or body.get('TransactionId')
